 ...
 */
#ifndef GUARD_KERNEL_INCLUDE_DEV_PCI_H
#define GUARD_KERNEL_INCLUDE_DEV_PCI_H 1
...
FORCELOCAL u32 FCALL pci_read(pci_addr_t base, pci_reg_t reg) { assert(!(base&PCI_ADDR_REGMASK)); return pci_readaddr(base|reg); }
FORCELOCAL void FCALL pci_write(pci_addr_t base, pci_reg_t reg, u32 value) { assert(!(base&PCI_ADDR_REGMASK)); pci_writeaddr(base|reg,value); }
#endif /* __CC__ */
...
#ifdef __CC__
struct pci_device {
    LIST_NODE(struct pci_device) pd_chain;      /* Chain of discovered PCI devices. */
...
DATDEF SLIST_HEAD(struct pci_device) pci_list;

/* Return a PCI device located at the given
 * `address', or NULL if no such device exists. */
FUNDEF struct pci_device *KCALL lookup_pci_device(pci_addr_t address);

/* Enumerate all PCI devices. */
#define PCI_FOREACH(dev) \
       LIST_FOREACH(dev,pci_list,pd_chain)
...
#endif /* !GUARD_KERNEL_INCLUDE_DEV_PCI_H */
