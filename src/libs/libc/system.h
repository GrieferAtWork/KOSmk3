...
INTDEF errno_t LIBCCALL Esys_sigaction(int sig, struct sigaction const *act, struct sigaction *oact, size_t sigsetsize);
INTDEF errno_t LIBCCALL Esys_sigprocmask(int how, __sigset_t const *set, __sigset_t *oldset, size_t sigsetsize);
INTDEF int LIBCCALL Esys_sigtimedwait(__sigset_t const *uthese, siginfo_t *uinfo, struct timespec64 const *uts, size_t sigsetsize);
INTDEF errno_t LIBCCALL Esys_sigpending(__sigset_t *uset, size_t sigsetsize);
INTDEF errno_t LIBCCALL Esys_sigsuspend(__sigset_t const *unewset, size_t sigsetsize);
...
INTDEF errno_t LIBCCALL Esys_waitid(int which, pid_t upid, siginfo_t *infop, int options, struct rusage *ru);
...
INTDEF void *LIBCCALL Esys_xmmap(int version, struct mmap_info const *data);
...
INTDEF ssize_t LIBCCALL sys_xcapture_traceback(struct cpu_context *ctx, unsigned int num_skipframes, void **ptb, size_t ptb_count);
...
INTDEF ssize_t LIBCCALL Esys_xppoll(struct pollfd *ufds, size_t nfds, struct pollfutex *uftx, size_t nftx, struct timespec64 const *tsp, void *sig);
...
INTDEF size_t LIBCCALL Xsys_xppoll(struct pollfd *ufds, size_t nfds, struct pollfutex *uftx, size_t nftx, struct timespec64 const *tsp, void *sig);
