...
struct mmap_virt {
    ...
    union __ATTR_PACKED {
        __FS_TYPE(pos) mv_off;   /* [valid_if(!MAP_ANONYMOUS)] Offset into 'mv_file' where mapping starts. */
...
#if __KOS_VERSION__ >= 300
struct mmap_ushare {
    __UINT32_TYPE__    mu_name;  /* User-share segment ID (One of `USHARE_*' from <kos/ushare.h>) */
...
__REDIRECT_EXCEPT_(__LIBC,__PORT_KOSONLY_ALT(mmap),void *,__LIBCCALL,xmmap,(struct mmap_info const *__data),xmmap1,(__data))
...
#if __KOS_VERSION__ >= 300
/* @param: flags: Set of `XUNMAP_*'; @return: * : Number of modified bytes */
__REDIRECT_EXCEPT(__LIBC,__PORT_KOSONLY_ALT(mprotect),__EXCEPT_SELECT(size_t,__ssize_t),__LIBCCALL,xmprotect,(void *__addr, size_t __len, int __protmask, int __protflag, int __flags, void *__tag),(__addr,__len,__protmask,__protflag,__flags,__tag))
#endif
